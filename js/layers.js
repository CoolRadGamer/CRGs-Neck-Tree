addLayer("cn", {
    ...
    gainMult() { // Calculate the multiplier for main currency from bonuses
	    ...
	    if (hasUpgrade('cn', 12)) mult = mult.plus(100).pow(player.cn.points.pow(0.1))
	     if (hasUpgrade('cn', 13)) mult = mult.tetrate(player.cn.points.tetrate(0.001).plus(1))
	    if (hasUpgrade('cn', 13)) mult = mult.tetrate(player.cn.points.tetrate(0.002).plus(1))
	    return mult
    },
